  ...
  async syncAllData(): Promise<{ success: boolean; error?: string }> {
    ...
    try {
      ...
      try {
        ...
        console.warn('⚠️ Erro ao sincronizar comuns (continuando...):', error instanceof Error ? error.message : error);
      }

      try {
        await supabaseDataService.syncCargosToLocal();
      } catch (error) {
        console.warn('⚠️ Erro ao sincronizar cargos (continuando...):', error instanceof Error ? error.message : error);
      }

      try {
        await supabaseDataService.syncInstrumentosToLocal();
      } catch (error) {
        console.warn('⚠️ Erro ao sincronizar instrumentos (continuando...):', error instanceof Error ? error.message : error);
      }
      ...
      try {
        ...
        console.warn('⚠️ Erro ao sincronizar registros pendentes (continuando...):', error instanceof Error ? error.message : error);
      }
      ...
      try {
        ...
        try {
          ...
          if (sheetsResult.success) {
            ...
            console.warn(`⚠️ Falha ao enviar ${registro.id} para Google Sheets:`, sheetsResult.error);
          }
        } catch (sheetsError) {
        ...
        try {
          ...
          if (registro.id) {
            ...
            console.log(`✅ Registro ${registro.id} sincronizado com sucesso (Google Sheets: ${sheetsSuccess ? 'OK' : 'Falhou'}, Supabase: OK)`);
          }
        } catch (supabaseError) {
          ...
          if (sheetsSuccess) {
            console.warn(`⚠️ Registro ${registro.id} enviado para Google Sheets mas falhou no Supabase - mantendo como pending`);
          }
        }
      } catch (error) {
  ...
  async createRegistro(registro: RegistroPresenca): Promise<{ success: boolean; error?: string }> {
      ...
      try {
        ...
        if (comum && cargo) {
          ...
          if (registro.pessoa_id.startsWith('manual_')) {
            ...
            if (pessoa) {
              nomeCompleto = (pessoa.nome_completo || `${pessoa.nome} ${pessoa.sobrenome}`).trim().toUpperCase();
    ...
    try {
      ...
      const [comuns, cargos, pessoas] = await Promise.all([
        supabaseDataService.getComunsFromLocal(),
        supabaseDataService.getCargosFromLocal(),
        supabaseDataService.getPessoasFromLocal(registro.comum_id, registro.cargo_id, registro.instrumento_id || undefined),
      ]);
    ...
    if (isOnline) {
      try {
        ...
        const sessionValid = await authService.isSessionValid();
        if (!sessionValid) {
          console.warn('⚠️ Sessão inválida, registro salvo localmente para sincronização posterior');
          return {
            success: true,
        ...
        try {
          ...
          if (createdRegistro) {
            ...
            console.log(`✅ Registro sincronizado com sucesso (Google Sheets: ${sheetsSuccess ? 'OK' : 'Falhou'}, Supabase: OK)`);
            return { success: true };
          } else {
          ...
          if (supabaseError instanceof Error && supabaseError.message.includes('DUPLICATA_BLOQUEADA')) {
          ...
          // Se Google Sheets foi bem-sucedido mas Supabase falhou, manter como pending
          if (sheetsSuccess) {
            console.warn('⚠️ Registro enviado para Google Sheets mas falhou no Supabase - mantendo como pending');
          }
