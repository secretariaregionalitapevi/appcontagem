...
export const offlineSyncService = {
  ...
  // 🚨 FUNÇÃO EXATA DO BACKUPCONT: processarFilaLocal
  async processarFilaLocal(): Promise<void> {
    try {
      // Buscar fila exatamente como BACKUPCONT
      ...
      // Testa conectividade antes de processar (como BACKUPCONT)
      const conectividadeOK = await this.isOnline();
      ...
      for (let i = 0; i < registros.length; i++) {
        ...
        try {
          ...
          // Tenta enviar para Google Sheets (como BACKUPCONT)
          const sheetsResult = await googleSheetsService.sendRegistroToSheet(item);
          if (sheetsResult.success) {
            console.log(`✅ Item ${i + 1}: Google Sheets OK`);
          } else {
            throw new Error(sheetsResult.error || 'Erro ao enviar para Google Sheets');
          }
          
          // Tenta enviar para Supabase (como BACKUPCONT)
          try {
          ...
          // Incrementa tentativas (como BACKUPCONT)
          const tentativas = (item as any).tentativas || 0;
          (item as any).tentativas = tentativas + 1;
          
          // Se já tentou 3 vezes, remove da fila (como BACKUPCONT)
      ...
      // Atualiza fila removendo itens processados (como BACKUPCONT)
